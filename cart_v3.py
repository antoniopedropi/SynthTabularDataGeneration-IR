...
class RarityWeightedCARTSynthesizer:
    ...
    def _compute_global_rarity(self, bandwidth=1, alpha=1.5):
    ...
    def generate_synthetic_data(self, 
                                ...
                                bandwidth=1.0,
                                alpha=1.0):
        self._compute_global_rarity(bandwidth=bandwidth, alpha=alpha)
...
augmented_df, synthetic_df = synth.generate_synthetic_data(
    ...
    resample_size=200,
    bandwidth=0.5,         # KDE smoothing
    alpha=2.0              # rarity emphasis
)
...
weights = dw.fit(df["Rings"].values.reshape(-1, 1))

plt.figure(figsize=(10, 6))
plt.scatter(df["Rings"], weights, c=weights, cmap='viridis', alpha=0.8)

plt.colorbar(label="Density-based weight")
plt.xlabel("Rings")
plt.ylabel("Weight (inverse density)")
plt.title("Rings vs. KDE-based Rarity Weight")
plt.grid(True)
plt.tight_layout()
plt.show()